...
import { getDailyForecasts, getHourlyForecasts } from '../../src/services/forecast.service';
const mockGetDaily = getDailyForecasts as jest.Mock;
const mockGetHourly = getHourlyForecasts as jest.Mock;

describe('Forecast routes', () => {
  beforeEach(() => jest.clearAllMocks());

  it('GET /api/forecasts/:locationId returns daily forecasts', async () => {
    mockGetDaily.mockResolvedValue([{ id: 'daily-1', forecast_date: '2026-05-03' }]);
  ...
  it('GET /api/forecasts/:locationId/hourly returns hourly data', async () => {
    ...
    const res = await request(app).get('/api/forecasts/loc-1/hourly?dailyId=daily-1');
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
  });
});
